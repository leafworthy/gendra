...
{
	public Stack<MoveCommand> UndoCommands = new();
	public Stack<MoveCommand> RedoCommands = new();
	...
	private void OnItemStopDragging(Item item)
	{
		...
		if (_originalPosition == item.GetInventoryPosition() && _originalDirection == item.GetDirection() &&
		    _originalInventory == item.GetInventory())
		{
